  ...
  return (
    <View className="flex-1">
      <View className="space-y-6">
        ...
        <FormField control={control} name="experience_text" label="Experience *" placeholder="Tell employers about your work experience..." multiline numberOfLines={4} textAlignVertical="top" />

        <View>
          <Text className="text-white text-sm font-medium mb-2">Skills * (max 5)</Text>
          <View className="flex-row mb-2">
            <TextField className="flex-1 mr-2" placeholder="Add a skill..." value={skillInput} onChangeText={setSkillInput} onSubmitEditing={addSkill} />
            <Button title="Add" onPress={addSkill} disabled={!skillInput.trim() || skills.length >= 5} />
          </View>
          <View className="flex-row flex-wrap">
            {skills.map((skill, index) => (
              <View key={index} className="bg-indigo-600 px-3 py-2 rounded-full mr-2 mb-2 flex-row items-center">
          ...
          {errors.skills && <Text className="text-red-400 text-xs mt-1">{errors.skills.message}</Text>}
        </View>

        <FormField control={control} name="availability_text" label="Availability *" placeholder="E.g., Weekdays after 5pm, weekends anytime..." />
        ...
        <View>
              ...
              <View>
                {WORK_RIGHTS_OPTIONS.map((right) => (
                  <Pressable
                    ...
                    className={`px-4 py-3 mb-2 rounded-lg border ${value === right.value ? 'bg-indigo-600/20 border-indigo-600' : 'bg-slate-900 border-slate-800'}`}
                  >
          ...
          />
          {errors.work_rights && <Text className="text-red-400 text-xs mt-1">{errors.work_rights.message}</Text>}
        </View>
      </View>
    </View>
  );
