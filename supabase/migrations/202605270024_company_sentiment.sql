...
create table company_sentiments (
  ...
  source text not null,                              -- e.g. 'glassdoor', 'linkedin', 'seek_review'
  sentiment_label text not null
    check (sentiment_label in ('positive', 'neutral', 'negative')),
  score float not null
    check (score >= -1.0 and score <= 1.0),
  snippet text,                                      -- the actual text that was scored
...
create policy "company_sentiments_insert_service" on company_sentiments for insert
  with check (auth.role() = 'service_role');

-- RLS: authenticated users can read
create policy "company_sentiments_select_auth" on company_sentiments for select
...
-- RLS on materialised view: authenticated users can read
-- (Materialised views in Supabase/PG inherit table-level RLS; we create
--  explicit policies so the view is accessible the same way.)
create policy "company_aggregates_select_auth" on company_aggregates for select
  using (auth.role() = 'authenticated');
...
as $$
begin
  refresh materialized view concurrently company_aggregates;
end;
$$;
